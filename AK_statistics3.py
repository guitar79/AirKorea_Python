...
for i in sorted(os.listdir(drbase+drin)):
    ...
    if i[-4:] == '.csv':
        ...
        total_mean_value = f.mean().reshape(1,11)
        total_var_value = f.var().reshape(1,11)
        total_std_value = f.std().reshape(1,11)
        total_max_value = f.max().reshape(1,11)
        total_min_value = f.min().reshape(1,11)
