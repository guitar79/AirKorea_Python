...
save_dir_name = '../AirKorea_monthly_from_hourly_data/'
...
for OBS_code in OBS_codes:
    ...
    while processing_sdatetime < end_datetime :
        processing_edatetime = processing_sdatetime + relativedelta(months=+1)
    ...
    OBS_code_daily_df.to_csv('{0}{1}_monthly_from_hourly_{2}_{3}.csv'.format(save_dir_name, OBS_code, start_datetime.strftime("%Y-%m-%d"), end_datetime.strftime("%Y-%m-%d")), index=False)
    print('{0}{1}_monthly_from_hourly_{2}_{3}.csv is created...'.format(save_dir_name, OBS_code, start_datetime.strftime("%Y-%m-%d"), end_datetime.strftime("%Y-%m-%d")))
