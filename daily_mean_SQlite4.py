...
def daily_statistics_by_ocode(conn, ocode, day):
	...
	for i in range(0,24):
		...
		if(f.iloc[i]['NO2']!=''):
			...
			top5+=1
        if(f.iloc[i]['PM25']!=''):
			PM25lalala[top5]=1000*float(f.iloc[i]['NO2'])
			top6+=1
	...
	with open('%s%sstatistics_%s.csv' %(drbase,drout,ocode), 'a') as o:
		print('%s,,%s,,'sum',%d,%d,%d,%d,%d,%d,' % (day,ocode,SO2lalala.mean()/1000,COlalala.mean()/1000,O3lalala.mean()/1000,NO2lalala.mean()/1000,PM10lalala.mean()/1000,PM25lalala.mean()/1000), file=o)
