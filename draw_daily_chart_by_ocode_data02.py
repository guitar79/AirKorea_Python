...
#NO2 ocodes = ['111264','111242','111232','111143','111202','111122'] 
ocodes = ['131343','131411','437112','111213','533116','131442'] 

#'SO2' -5, 'CO' -6, 'O3' -7, 'NO2' -8, 'PM10' -9, 'PM25' -10
data = 9
...
for ocode in sorted(ocodes): 
    ...
    if len(rows) == 0:
        print('result is empty at %s' %(ocode))
    else:
        rows = np.array(rows)
        print(rows)
        ...
        pylab.plot(xdata,rows[:,data],"g")
        pylab.suptitle('Variation of PM10 daily mean concentration at site(%s)' %(ocode), fontsize=20)
        pylab.xlabel('date', fontsize=15)
        pylab.ylabel('daily mean concentration(ppm)', fontsize=15)
