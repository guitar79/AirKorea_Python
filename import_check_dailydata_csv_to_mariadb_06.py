...
#db connect
'''
n= pymysql.connect(host=db_host, user=db_user, password=db_pass, db=db_name,\
...
conn.commit()
'''
check_row = open(drbase+'check_rows.log', 'a')
finish_rows = 0
    ...
    if i[-4:] == '.csv':
        ...
        for j in range(0,(len(raw_lists)-1)):
            ...
            if len(row)==10 :
                ...
                print(i, j, len(row), row)
                check_row.write(i, j, row)
                '''
